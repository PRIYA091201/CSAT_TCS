...
|-------|--------|-----------------|
| Frontend | React 18 + TypeScript + Tailwind CSS | Yes. No Next.js, no Remix, no Angular, no Vue. |
| Build tool | Vite 5 | Yes. No webpack, no Create React App, no Parcel. |
| UI components | shadcn/ui (Radix UI) | Yes. Copy/paste into codebase — NOT installed as a package dependency. |
| Charts | Recharts | Yes. Admin dashboard charts only. Already in admin-dashboard package.json. |
| CSS utilities | clsx + tailwind-merge | Yes. Used via `cn()` helper in lib/utils.ts. Already in admin-dashboard package.json. |
...
| State management | TanStack Query v5 for server state, React useState/useReducer for local state | Yes. No Redux, no Zustand, no MobX. |
| Validation | Zod v3 | Yes. No Yup, no Joi. |
| QR generation | qrcode.react | Yes. Client-side only. |
| Package manager | pnpm 10 | Yes. No npm, no yarn. |
| Monorepo | pnpm workspaces | Yes. Root `pnpm-workspace.yaml` defines `apps/*` and `packages/*`. |
...
## Admin user setup (enforced)

Admin and MD users **cannot self-register**. They must be created via SQL in the Supabase dashboard. The setup SQL is in `supabase/create_admin_user.sql`.

**Run once before first login:**
1. Go to: `https://supabase.com/dashboard/project/mxfuufwztkgirdjjxkci/sql/new`
2. Paste and run: `supabase/create_admin_user.sql`
3. Default credentials — change after first login:
   - Admin: `admin@chennaisilks.com` / `Admin@1234`
   - MD: `md@chennaisilks.com` / `MD@1234`

**Why SQL and not the Supabase Auth UI:**
The Supabase Auth UI creates users but does NOT set `raw_app_meta_data.role`. Without the role, the frontend redirects the user back to `/login` even after a successful password authentication. The SQL sets both password and role atomically.

**The role MUST be in `raw_app_meta_data`, NOT `raw_user_meta_data`:**
- `raw_app_meta_data` — set by server/admin only. Cannot be modified by the user. Used in RLS policies.
- `raw_user_meta_data` — can be modified by the user. Never trust for RBAC.

**Agent must:**
- Never create admin users via the Supabase Auth dashboard UI alone — role won't be set.
- Always use `supabase/create_admin_user.sql` or the equivalent SQL pattern.
- When testing auth, verify `raw_app_meta_data->>'role'` returns `'admin'` or `'md'` before debugging frontend.
- If login succeeds but dashboard redirects to `/login`: the role is missing. Run the role UPDATE query.

**Diagnosis query (run in Supabase SQL editor):**
```sql
SELECT email, raw_app_meta_data->>'role' AS role, email_confirmed_at IS NOT NULL AS confirmed
FROM auth.users WHERE email IN ('admin@chennaisilks.com', 'md@chennaisilks.com');
```
...
------------------------+-------+----------
admin@chennaisilks.com  | admin | true
md@chennaisilks.com     | md    | true
```
